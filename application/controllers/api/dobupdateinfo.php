<?php
class Dobupdateinfo extends CI_Controller {	
	...
	function index()
	{
		...
		## 	Validate all data
			if($request_id)
			{
				...
				$sql="SELECT SUB.`int_subscriber_key` AS t_subscriber_id, SUB.`tx_name`, SUB.`tx_mobile`,  
CEIL(DATEDIFF(CURDATE(),SUB.dtt_week_base)/7) AS weeks,
...
WHERE `tx_healthworker_id`='".$username."' AND `tx_status`='Registered'
AND (SUB.`int_subscriber_type_key` = 1 OR SUB.`int_subscriber_type_key` = 7 OR SUB.`int_subscriber_type_key` = 10)
AND (CEIL(DATEDIFF(CURDATE(),SUB.dtt_week_base)/7) >= ".$dob_limit_in_week.") 
ORDER BY `SUB`.`dtt_week_base` ASC, `SUB`.`int_subscriber_key` ASC;";
